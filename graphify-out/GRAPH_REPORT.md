...
## Summary
- 1341 nodes · 3346 edges · 101 communities (80 shown, 21 thin omitted)
- Extraction: 65% EXTRACTED · 35% INFERRED · 0% AMBIGUOUS · INFERRED: 1185 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `afbbebd6`
...
1. `PyatsSnapshot` - 158 edges
2. `PyatsJob` - 153 edges
3. `PyatsSnapshotDiff` - 143 edges
4. `PyatsComplianceRun` - 132 edges
5. `PyatsCredential` - 128 edges
6. `PyatsGoldenConfig` - 126 edges
7. `SnapshotKindChoices` - 99 edges
8. `SnapshotTriggerChoices` - 84 edges
9. `SnapshotStatusChoices` - 71 edges
10. `PyatsParserCatalog` - 57 edges
...
Cohesion: 0.07
Nodes (52): PyatsComplianceRun, PyatsGoldenConfig, PyatsJob, A golden / reference running-config for a NetBox Device (Phase 4, ATW-15)., True if this golden config was promoted from a snapshot row., One compliance check result: golden config vs. captured snapshot (Phase 4, ATW-1, Map result to a NetBox color label for table badges., True if the diff found any added/removed/changed leaves (drift). (+44 more)
...
Cohesion: 0.17
Nodes (39): ComplianceResultChoices, CredentialProtocolChoices, CredentialScopeChoices, DiffStatusChoices, GoldenConfigSourceChoices, PyatsJobStatusChoices, PyatsJobTypeChoices, How a :class:`PyatsGoldenConfig` row was authored (Phase 4, ATW-15).      ``manu (+31 more)

### Community 5 - "run_diff_job"
Cohesion: 0.08
Nodes (35): BaseException, batch_capture_job(), capture_snapshot_job(), _create_pyats_job(), enqueue_batch_capture(), enqueue_capture(), enqueue_compliance(), enqueue_diff() (+27 more)
...
### Community 7 - "run_compliance"
Cohesion: 0.09
Nodes (10): _normalize_lines(), Normalize a running-config text into a list of comparable lines.      Drops blan, Compare a golden config text against a snapshot's raw config text and classify., run_compliance(), Exercise the exact path the RQ job runs: golden text → snapshot raw text.      T, TestComplianceResultSizeBytes, TestCompliant, TestDrift (+2 more)
...
Cohesion: 0.13
Nodes (17): Platform-support decision for the device-page PyATS panel (ATW-184).  Pure-Pytho, Return ``(platform_supported, os_value)`` for the device-page panel.      Combin, resolve_panel_platform_support(), _capture_url_for_device(), _compliance_url_for_device(), _diff_url_for_device(), Template extensions injecting the PyATS tab into the NetBox Device page.  Phase, Return the POST URL for the device-page capture form. (+9 more)
...
Cohesion: 0.10
Nodes (20): PyatsCredential, Encrypt and store the device password (ciphertext only)., Decrypt and return the device password (plaintext)., Encrypt and store the enable/privileged password (ciphertext only)., Decrypt and return the enable/privileged password (plaintext)., A plugin-local, encrypted credential for connecting to a device via pyATS., Meta, PyatsComplianceRunTable (+12 more)

### Community 13 - "choices.py"
Cohesion: 0.08
Nodes (15): Choice sets for the netbox-pyats plugin., ComplianceResult, Compliance engine — golden config vs. snapshot raw config diff (Phase 4, ATW-15), Length of the JSON-serialized ``diff`` payload, in bytes., True if the diff found any added/removed/changed leaves (drift)., Outcome of a single :func:`run_compliance` call.      The RQ job (:func:`netbox_, Migration, Migration (+7 more)
...
Cohesion: 0.11
Nodes (19): PyatsSnapshot, One captured config/state/full snapshot for a NetBox Device.      Populated by t, Map status to a NetBox color label for table badges., True if this snapshot row carries parser warnings / error context., PyatsComplianceRunIndex, PyatsCredentialIndex, PyatsGoldenConfigIndex, PyatsJobIndex (+11 more)

### Community 16 - "SnapshotTriggerChoices"
Cohesion: 0.12
Nodes (14): Who/what triggered a snapshot capture.      ``user`` captures are initiated from, SnapshotTriggerChoices, _AppendOnlyListViewsBase, TestCase, PyatsComplianceRunListViewRenderTest, PyatsJobListViewRenderTest, PyatsSnapshotDiffListViewRenderTest, PyatsSnapshotListViewRenderTest (+6 more)

### Community 17 - "What You Must Do When Invoked"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 18 - "PyatsComplianceRunModelTest"
Cohesion: 0.16
...
### Community 20 - "SnapshotKindChoices"
Cohesion: 0.15
Nodes (12): What a :class:`PyatsSnapshot` captures from a device.      ``config`` runs parse, Outcome of a snapshot capture attempt.      ``success`` means a JSONB ``data`` p, SnapshotKindChoices, SnapshotStatusChoices, NetBox background jobs for the netbox-pyats plugin.  Phase 2 (ATW-13) ships the, DiffJobPyatsJobPlumbingTest, Tests for the PyatsJob model + job-callable side effects + batch summary (Phase, ADR-0005 §3 plumbing for ``run_diff_job`` (Phase 5, ATW-16). (+4 more)
...
### Community 25 - "platform_to_pyats_os"
Cohesion: 0.31
...
### Community 29 - "ADR-0006: PR-body hygiene — no Paperclip control-plane metadata in public GitHub artifacts"
Cohesion: 0.14
...
### Community 32 - "Remote access to the dev NetBox UI over Tailscale"
Cohesion: 0.15
...
### Community 35 - "test_graphify_scrub_guard.py"
Cohesion: 0.33
...
### Community 36 - "test_supported_platforms.py"
Cohesion: 0.15
...
### Community 46 - "Graphify MCP HTTP server — multi-host / shared-service runbook"
Cohesion: 0.18
...
### Community 47 - "PyatsCredentialModelTest"
Cohesion: 0.18
Nodes (3): TestCase, PyatsCredentialModelTest, Field-level encryption and validation behavior of PyatsCredential.

### Community 48 - "ADR-0003: NetBox 4.6 migration dependencies and worker build toolchain"
Cohesion: 0.20
...
### Community 49 - "ADR-0004: Compliance golden-config comparison shape"
Cohesion: 0.20
...
### Community 50 - "ADR-0005: PyatsJob unified job-tracking model + status vocabulary extension"
Cohesion: 0.20
...
Cohesion: 0.20
Nodes (6): Exception, ParserNotFound, Tests for :mod:`netbox_pyats.capture`.  Pure-Python: exercises the snapshot capt, Duck-type stand-in for ``genie.libs.parser.utils.common.ParserNotFound``.      T, TestCaptureError, TestFullCapture
...
- **Why does `PyatsCredential` connect `PyatsCredential` to `PyatsJob`, `views.py`, `build_testbed`, `DiffStatusChoices`, `PyatsCredentialViewTest`, `PyatsCredentialForm`, `PyatsSnapshotDiff`, `models.py`, `PyatsSnapshot`, `SnapshotTriggerChoices`, `PyatsCredentialModelTest`, `SnapshotKindChoices`, `DeviceBulkCaptureView`, `PyatsCredentialAPITest`, `testbed.py`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `SnapshotStatusChoices` connect `SnapshotKindChoices` to `PyatsJob`, `DiffStatusChoices`, `CaptureResult`, `resolve_panel_platform_support`, `capture_snapshot`, `PyatsSnapshotDiff`, `PyatsCredential`, `choices.py`, `models.py`, `PyatsSnapshot`, `SnapshotTriggerChoices`, `PyatsComplianceRunModelTest`, `PyatsGoldenConfigAPITest`, `PyatsComplianceRunViewTest`, `capture.py`, `PyatsSnapshotDiffModelTest`, `test_capture.py`, `PyatsJobModelTest`, `PyatsSnapshotModelTest`, `PyatsCredentialForm`, `TestStateCapture`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Why does `PyatsJob` connect `PyatsJob` to `views.py`, `refresh_parser_catalog_for_os`, `DiffStatusChoices`, `run_diff_job`, `PyatsCredentialForm`, `CaptureResult`, `PyatsSnapshotDiff`, `PyatsCredential`, `models.py`, `PyatsSnapshot`, `SnapshotTriggerChoices`, `SnapshotKindChoices`, `PyatsJobModelTest`, `DeviceBulkCaptureView`?**
  _High betweenness centrality (0.048) - this node is a cross-community bridge._
- **Are the 118 inferred relationships involving `PyatsSnapshot` (e.g. with `Meta` and `PyatsComplianceRunSerializer`) actually correct?**
  _`PyatsSnapshot` has 118 INFERRED edges - model-reasoned connections that need verification._
- **Are the 110 inferred relationships involving `PyatsJob` (e.g. with `Meta` and `PyatsComplianceRunSerializer`) actually correct?**
  _`PyatsJob` has 110 INFERRED edges - model-reasoned connections that need verification._
- **Are the 112 inferred relationships involving `PyatsSnapshotDiff` (e.g. with `Meta` and `PyatsComplianceRunSerializer`) actually correct?**
  _`PyatsSnapshotDiff` has 112 INFERRED edges - model-reasoned connections that need verification._
- **Are the 102 inferred relationships involving `PyatsComplianceRun` (e.g. with `Meta` and `PyatsComplianceRunSerializer`) actually correct?**
  _`PyatsComplianceRun` has 102 INFERRED edges - model-reasoned connections that need verification._