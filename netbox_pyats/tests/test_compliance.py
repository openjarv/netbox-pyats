...
class TestErrorInputs:
    ...
    def test_diff_engine_error_propagates_as_compliance_error(self):
        # The diff engine returns status="error" for non-dict top-level
        # payloads. run_compliance must surface that as a compliance error.
        # We reach the diff engine's error path by passing dicts whose values
        # are non-dict/list types — but the diff engine only errors on
        # non-dict top-level inputs. Since run_compliance pre-checks for empty
        # inputs and the diff engine itself handles dict inputs gracefully,
        # the realistic error-from-engine path is exercised through the
        # job-level tests with malformed persisted JSONB. Here we verify the
        # classification logic for valid dicts is correct (compliant/drift
        # classes above cover the happy paths), and that the error-class
        # shape is correct (diff={}, summary={}, warnings non-empty).
