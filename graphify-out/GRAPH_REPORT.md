...
## Summary
- 1822 nodes · 4894 edges · 145 communities (99 shown, 46 thin omitted)
- Extraction: 61% EXTRACTED · 39% INFERRED · 0% AMBIGUOUS · INFERRED: 1910 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a6d6697d`
...
### Community 3 - "DiffStatusChoices"
Cohesion: 0.13
Nodes (48): CredentialProtocolChoices, CredentialScopeChoices, DiffStatusChoices, GoldenConfigSourceChoices, PyatsJobStatusChoices, PyatsJobTypeChoices, How a :class:`PyatsGoldenConfig` row was authored (Phase 4, ATW-15).      ``manu, How a credential is assigned.      ``device`` credentials attach to a single Net (+40 more)
...
Cohesion: 0.10
Nodes (15): _capture_config(), _capture_parse(), capture_snapshot(), Run parser-based config capture on a connected pyATS Device.      Uses ``pyats.u, Run on-demand parser capture for an explicit, user-supplied command list.      T, Capture a snapshot from a single, already-connected pyATS Device.      This is t, FakePyatsDevice, kind='parse' runs device.parse() per user-supplied command and writes     the sa (+7 more)
...
Cohesion: 0.07
Nodes (24): ComplianceModeChoices, ComplianceResultChoices, Choice sets for the netbox-pyats plugin., Outcome of a compliance run (Phase 4, ATW-15).      ``compliant`` means the devi, How :func:`netbox_pyats.compliance.run_compliance` compares configs.      ``orde, ComplianceResult, Compliance engine — golden config vs. snapshot raw config diff (Phase 4, ATW-15), Outcome of a single :func:`run_compliance` call.      The RQ job (:func:`netbox_ (+16 more)
...
### Community 13 - "DeviceDiffForm"
Cohesion: 0.12
Nodes (11): DeviceDiffForm, Form backing the device-page "Diff two snapshots" picker (Phase 3).      Posted, Initialize the form with an optional device scope.          Args:             de, Require at least one of ``commands`` or ``manual_command``.          The parse j, DeviceDiffFormKindFilterTest, DeviceDiffViewKindFilterTest, _make_snapshot(), Tests for the ``DeviceDiffForm`` kind filter (ATW-241 child 4 / ATW-252).  NetBo (+3 more)
...
Cohesion: 0.11
Nodes (15): Outcome of a snapshot capture attempt.      ``success`` means a JSONB ``data`` p, SnapshotStatusChoices, _AppendOnlyListViewsBase, TestCase, PyatsComplianceRunListViewRenderTest, PyatsJobListViewRenderTest, PyatsSnapshotDiffListViewRenderTest, PyatsSnapshotListViewRenderTest (+7 more)
...
### Community 23 - "run_parser_catalog_refresh_schedules_job"
Cohesion: 0.13
...
Cohesion: 0.11
Nodes (7): DeviceRefreshCatalogViewTest, View tests for :class:`views.DeviceRefreshCatalogView` (ATW-250)., DiffJobPyatsJobPlumbingTest, PyatsJobModelTest, ADR-0005 §3 plumbing for ``run_diff_job`` (Phase 5, ATW-16)., Persistence + helpers for PyatsJob (Phase 5, ATW-16)., TestCase

### Community 25 - "_cred_resolver_factory"
Cohesion: 0.22
...
### Community 27 - "diff.py"
Cohesion: 0.18
Nodes (18): Any, _diff_dict(), _diff_list(), _diff_value(), _flatten_node(), _join_path(), _leaf_type(), _node_status() (+10 more)

### Community 28 - "run_diff_job"
Cohesion: 0.17
Nodes (19): BaseException, capture_snapshot_job(), _finish_success(), _mark_running(), parse_commands_job(), _persist_error_row(), RQ worker entry point — run on-demand parses and persist the snapshot.      NetB, Set a :class:`PyatsJob` to ``running`` with ``started_at=now()``.      Called at (+11 more)

### Community 29 - "Dev environment bring-up"
Cohesion: 0.11
...
### Community 30 - "EncryptDecryptTest"
Cohesion: 0.17
...
### Community 31 - "SnapshotKindChoices"
Cohesion: 0.13
Nodes (10): Snapshot capture logic — the pyATS/Genie work, isolated from NetBox/RQ.  :func:`, What a :class:`PyatsSnapshot` captures from a device.      ``config`` runs parse, SnapshotKindChoices, Integration render test for the side-by-side diff table (ATW-524/ATW-525).  Repl, DeviceBulkCaptureViewTest, Tests for the PyatsJob model + job-callable side effects + batch summary (Phase, The device-list bulk "PyATS capture" view renders its confirmation     form (Pha, Tests for :class:`netbox_pyats.models.PyatsSnapshot`.  Requires a running NetBox (+2 more)
...
### Community 33 - "test_pr_body_scrub_guard.py"
Cohesion: 0.18
...
Cohesion: 0.17
Nodes (15): _create_pyats_job(), enqueue_batch_capture(), enqueue_capture(), enqueue_compliance(), enqueue_diff(), enqueue_parse(), enqueue_refresh_parser_catalog(), NetBox background jobs for the netbox-pyats plugin.  Phase 2 (ATW-13) ships the (+7 more)
...
### Community 38 - "PyatsComplianceRunViewTest"
Cohesion: 0.12
...
### Community 39 - "ADR-0004: Compliance golden-config comparison shape"
Cohesion: 0.13
...
### Community 40 - "Contributing to netbox-pyats"
Cohesion: 0.13
...
### Community 43 - "dev-seed.sh"
Cohesion: 0.27
Nodes (10): cmd_build(), cmd_force_restore(), cmd_info(), cmd_remove(), cmd_restore(), die(), _restore(), dev-seed.sh script (+2 more)

### Community 44 - "ADR-0006: PR-body hygiene — no Paperclip control-plane metadata in public GitHub artifacts"
Cohesion: 0.15
...
### Community 45 - "Remote access to the dev NetBox UI over Tailscale"
Cohesion: 0.15
...
### Community 50 - "_extract_snapshot_raw"
Cohesion: 0.27
...
### Community 51 - "PyatsCredentialModelTest"
Cohesion: 0.17
...
### Community 53 - "test_supported_platforms.py"
Cohesion: 0.17
...
### Community 54 - "TestSupportedPlatformsMap"
Cohesion: 0.17
...
### Community 57 - "ADR-0002: Multi-vendor graceful degradation pattern"
Cohesion: 0.18
...
### Community 58 - "Graphify MCP HTTP server — multi-host / shared-service runbook"
Cohesion: 0.18
...
### Community 59 - "resolve_state_commands"
Cohesion: 0.25
...
### Community 60 - "ADR-0003: NetBox 4.6 migration dependencies and worker build toolchain"
Cohesion: 0.20
...
### Community 61 - "ADR-0005: PyatsJob unified job-tracking model + status vocabulary extension"
Cohesion: 0.20
...
### Community 65 - "TestStateCommandsInvariant"
Cohesion: 0.20
...
### Community 68 - "Scheduled parser-catalog refresh"
Cohesion: 0.22
...
### Community 75 - "capture_snapshot_for_netbox_device"
Cohesion: 0.25
...
### Community 76 - "testbed.py"
Cohesion: 0.29
Nodes (5): Platform-support decision for the device-page PyATS panel (ATW-184).  Pure-Pytho, NetBox → pyATS testbed bridge.  :func:`build_testbed` constructs a :class:`pyats, Return the PyatsCredential for a NetBox Device, or None if none exists.      v1, _resolve_credential(), Tests for the device-page panel platform-support decision (ATW-184).  Pure-Pytho

### Community 77 - "_build_device_entry"
Cohesion: 0.25
...
### Community 83 - "PULL_REQUEST_TEMPLATE.md"
Cohesion: 0.29
Nodes (6): Changes, Closing checklist, Linked issue, Notes for reviewers, Summary, Verification

### Community 84 - "PyatsCredentialForm"
Cohesion: 0.29
Nodes (3): PyatsCredentialForm, Create/edit form for a PyATS Credential.      Plaintext password/enable_secret a, Initialize the form, optionally pinning the ``commands`` choices.          Args:

### Community 85 - "TestStateCapture"
Cohesion: 0.29
...
### Community 89 - "ADR-0007: Device-page tab via `register_model_view` + `ObjectView`"
Cohesion: 0.33
...
- **Why does `SnapshotKindChoices` connect `SnapshotKindChoices` to `PyatsSnapshot`, `DiffStatusChoices`, `resolve_panel_platform_support`, `capture_snapshot`, `PyatsJob`, `ComplianceResultChoices`, `CaptureResult`, `DeviceDiffForm`, `run_capture_schedules_job`, `SnapshotStatusChoices`, `PyatsCredential`, `PyatsGoldenConfigAPITest`, `PyatsCaptureSchedule`, `TestCase`, `jobs.py`, `PyatsComplianceRunViewTest`, `PyatsComplianceRunModelTest`, `PyatsSnapshotDiffModelTest`, `resolve_state_commands`, `test_capture.py`, `PyatsSnapshotModelTest`, `PyatsCredentialForm`, `TestStateCapture`, `PyatsGoldenConfigModelTest`, `DiffTableRenderTest`?**
  _High betweenness centrality (0.063) - this node is a cross-community bridge._
- **Why does `PyatsJob` connect `PyatsJob` to `PyatsSnapshot`, `views.py`, `.get_status_color`, `DiffStatusChoices`, `jobs.py`, `.related_result`, `refresh_parser_catalog_for_os`, `ComplianceResultChoices`, `CaptureResult`, `DeviceDiffForm`, `SnapshotStatusChoices`, `PyatsCredential`, `PyatsCredentialForm`, `PyatsCaptureSchedule`, `TestCase`, `SnapshotKindChoices`?**
  _High betweenness centrality (0.041) - this node is a cross-community bridge._
- **Why does `PyatsSnapshot` connect `PyatsSnapshot` to `views.py`, `.get_status_color`, `DiffStatusChoices`, `.has_warnings`, `PyatsJob`, `ComplianceResultChoices`, `CaptureResult`, `DeviceDiffForm`, `SnapshotStatusChoices`, `PyatsCredential`, `PyatsGoldenConfigAPITest`, `PyatsCaptureSchedule`, `TestCase`, `run_diff_job`, `SnapshotKindChoices`, `jobs.py`, `PyatsComplianceRunViewTest`, `PyatsComplianceRunModelTest`, `PyatsSnapshotDiffModelTest`, `PyatsSnapshotModelTest`, `capture_snapshot_for_netbox_device`, `PyatsCredentialForm`, `PyatsGoldenConfigModelTest`, `DiffTableRenderTest`?**
