...
> **Phase 4 (this release):** everything in Phases 1–3, plus the compliance engine — `PyatsGoldenConfig` and `PyatsComplianceRun` models, a `run_compliance` RQ job that diffs a golden config against a captured snapshot and classifies the device as `compliant` / `drift` / `error` using the Phase 3 diff engine, a device-page "Run compliance" picker, and a compliance-run viewer (reusing the Phase 3 diff tree). See the [changelog](CHANGELOG.md) for the full feature history.
...
- **`PyatsComplianceRun` model + `run_compliance` RQ job** — from the device-page PyATS tab, pick a golden config and a captured config/full snapshot; the worker parses the golden text into a comparable dict, diffs it against the snapshot's parsed `data["config"]` using the Phase 3 `diff_snapshots` engine, and classifies the outcome as `compliant` (no added/removed/changed leaves), `drift` (any divergence), or `error` (missing/empty golden, unsupported snapshot, etc.). The run row stores the diff tree + summary counts + warnings and is **always created** so the operator sees the outcome in-line. v1 answers "does the running config match the golden?"; feature-specific compliance rules (e.g. "interface X must have MTU 1500") are deferred to v2.
...
- **`compliant`** — the structured diff between the golden and the snapshot's parsed `data["config"]` has no added/removed/changed leaves.
- **`drift`** — the diff has any added/removed/changed leaves; the diff tree shows *what* drifted.
- **`error`** — the golden text is empty, the snapshot has no `config` payload, or the snapshot is `unsupported`/`error`. The row is still created with a warning naming the missing input so the failure is visible in-line.

The engine reuses the Phase 3 `diff_snapshots` pure-Python diff so the same collapsible before/after tree renders the golden-vs-snapshot divergence in the compliance-run viewer. The golden's `config_text` is parsed into a comparable dict on the worker (where a connected pyATS Device can drive the Genie parser) or, when no device is available, from a caller-supplied line-oriented golden-config dict. v1 is pure-Python and NetBox/RQ/Genie-free at the engine layer so it is unit-testable without a device.
