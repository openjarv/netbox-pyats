...
3. Re-enter each `PyatsCredential`'s password / enable secret at **PyATS/Genie → Credentials →** the credential's edit page. Old ciphertext is unreadable; the re-entered plaintext is encrypted with the new key.
