...
Cohesion: 0.07
Nodes (66): PyatsCaptureSchedule, PyatsComplianceRun, PyatsParserCatalog, PyatsSnapshotDiff, An operator-authored intent to capture snapshots on a recurring schedule (ATW-43, One structured diff between two :class:`PyatsSnapshot` rows of a device.      Po, Map status to a NetBox color label for table badges., True if the diff found any added/removed/changed leaves. (+58 more)
...
Cohesion: 0.06
Nodes (42): BaseException, batch_capture_job(), capture_snapshot_job(), _create_pyats_job(), enqueue_batch_capture(), enqueue_capture(), enqueue_compliance(), enqueue_diff() (+34 more)

### Community 3 - "DiffStatusChoices"
Cohesion: 0.14
Nodes (51): ComplianceModeChoices, ComplianceResultChoices, CredentialProtocolChoices, CredentialScopeChoices, DiffStatusChoices, GoldenConfigSourceChoices, PyatsJobStatusChoices, PyatsJobTypeChoices (+43 more)

### Community 4 - "SnapshotKindChoices"
Cohesion: 0.07
Nodes (28): Choice sets for the netbox-pyats plugin., What a :class:`PyatsSnapshot` captures from a device.      ``config`` runs parse, Outcome of a snapshot capture attempt.      ``success`` means a JSONB ``data`` p, SnapshotKindChoices, SnapshotStatusChoices, is_encrypted_token(), Encryption helpers for the plugin-local PyATS credential store.  Field-level enc, Best-effort check that a stored value looks like a Fernet token.      Used by te (+20 more)
...
### Community 14 - "SnapshotTriggerChoices"
Cohesion: 0.10
Nodes (12): Who/what triggered a snapshot capture.      ``user`` captures are initiated from, SnapshotTriggerChoices, Meta, NetBox background jobs for the netbox-pyats plugin.  Phase 2 (ATW-13) ships the, DeviceBulkCaptureViewTest, DiffJobPyatsJobPlumbingTest, TestCase, PyatsJobModelTest (+4 more)

### Community 15 - "PyatsCaptureScheduleModelTest"
Cohesion: 0.11
Nodes (12): RQ worker entry point — dispatch captures for all enabled schedules.      Thin m, Dispatch captures for all enabled schedules (delegates to the wrapper)., run_capture_schedules_job(), Re-resolve :attr:`device_filter` to a Device queryset (run-time).          Thin, Re-resolve a ``device_filter`` JSON spec to a Device queryset at run time., _resolve_device_filter(), TestCase, PyatsCaptureScheduleModelTest (+4 more)
...
### Community 20 - "PyatsGoldenConfig"
Cohesion: 0.13
Nodes (19): PyatsGoldenConfig, A golden / reference running-config for a NetBox Device (Phase 4, ATW-15)., True if this golden config was promoted from a snapshot row., Meta, PyatsCaptureScheduleTable, PyatsComplianceRunTable, PyatsCredentialTable, PyatsGoldenConfigTable (+11 more)

### Community 21 - "PyatsJob"
Cohesion: 0.14
...
### Community 22 - "_AppendOnlyListViewsBase"
Cohesion: 0.11
...
### Community 26 - "resolve_panel_platform_support"
Cohesion: 0.23
...
### Community 31 - "test_pr_body_scrub_guard.py"
Cohesion: 0.16
...
### Community 36 - "ADR-0004: Compliance golden-config comparison shape"
Cohesion: 0.13
...
### Community 37 - "Contributing to netbox-pyats"
Cohesion: 0.13
...
Cohesion: 0.18
Nodes (7): Return the state-capture command list for a given pyATS ``os``.      Resolution, resolve_state_commands(), Tests for :mod:`netbox_pyats.capture`.  Pure-Python: exercises the snapshot capt, ATW-432: resolve_state_commands picks per-OS command sets from     PLUGINS_CONFI, TestCaptureError, TestFullCapture, TestResolveStateCommands

### Community 39 - "platform_to_pyats_os"
Cohesion: 0.31
...
### Community 46 - "test_compliance.py"
Cohesion: 0.15
...
### Community 48 - "PyatsComplianceRunModelTest"
Cohesion: 0.31
Nodes (3): TestCase, PyatsComplianceRunModelTest, Persistence and helper behavior of PyatsComplianceRun (Phase 4, ATW-15).
...
### Community 52 - "_extract_snapshot_raw"
Cohesion: 0.27
...
### Community 66 - "Scheduled captures"
Cohesion: 0.22
...
### Community 92 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 93 - "graphify-mcp-key.sh"
Cohesion: 0.53
Nodes (4): ensure_gitignored(), fingerprint_key(), graphify-mcp-key.sh script, usage()

### Community 94 - "netbox-pyats documentation"
Cohesion: 0.40
...
### Community 109 - "RunCaptureSchedulesJob"
Cohesion: 0.67
Nodes (3): JobRunner, Recurring dispatcher for capture schedules (ATW-433, ADR-0008).      A registere, RunCaptureSchedulesJob

## Knowledge Gaps
- **275 isolated node(s):** `entrypoint.sh script`, `GRAPHIFY_API_KEY`, `pyats-test-entrypoint.sh script`, `DJANGO_SETTINGS_MODULE`, `Migration` (+270 more)
...
- **Why does `PyatsSnapshot` connect `PyatsSnapshot` to `PyatsSnapshotDiff`, `views.py`, `extract_snapshot_raw_config`, `DiffStatusChoices`, `SnapshotKindChoices`, `PyatsCredential`, `CaptureResult`, `SnapshotTriggerChoices`, `PyatsGoldenConfig`, `PyatsJob`, `_AppendOnlyListViewsBase`, `DeviceDiffFormKindFilterTest`, `PyatsComplianceRunViewTest`, `PyatsGoldenConfigAPITest`, `PyatsComplianceRunModelTest`, `PyatsSnapshotDiffModelTest`, `PyatsSnapshotModelTest`, `DiffTableRenderTest`, `PyatsGoldenConfigModelTest`, `RunCaptureSchedulesJob`?**
  _High betweenness centrality (0.052) - this node is a cross-community bridge._
- **Why does `PyatsCredential` connect `PyatsCredential` to `PyatsSnapshotDiff`, `views.py`, `testbed.py`, `DiffStatusChoices`, `SnapshotKindChoices`, `TestbedBuildReport`, `PyatsCredentialAPITest`, `SnapshotTriggerChoices`, `PyatsSnapshot`, `PyatsGoldenConfig`, `PyatsJob`, `PyatsCredentialViewTest`, `PyatsCredentialModelTest`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `SnapshotKindChoices` connect `SnapshotKindChoices` to `PyatsSnapshotDiff`, `DiffStatusChoices`, `capture_snapshot`, `PyatsCredential`, `CaptureResult`, `group_snapshots_by_kind`, `SnapshotTriggerChoices`, `PyatsCaptureScheduleModelTest`, `PyatsSnapshot`, `PyatsGoldenConfig`, `PyatsJob`, `_AppendOnlyListViewsBase`, `DeviceDiffFormKindFilterTest`, `test_capture.py`, `PyatsComplianceRunViewTest`, `capture.py`, `PyatsGoldenConfigAPITest`, `PyatsComplianceRunModelTest`, `PyatsSnapshotDiffModelTest`, `PyatsSnapshotModelTest`, `DiffTableRenderTest`, `PyatsGoldenConfigModelTest`, `TestStateCapture`, `ParserNotFound`, `RunCaptureSchedulesJob`?**
