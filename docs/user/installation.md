...
After restart, a single top-level **PyATS/Genie** menu appears in the NetBox navigation — Genie Tools (Parse / Learn / Diff), Credentials, Snapshots, Golden Configs & Compliance, Automation, Parser Catalog, and Jobs & Platforms.

<img src="../screenshots/nav-pyats-menu.png" alt="NetBox navigation menu with the single top-level PyATS/Genie menu expanded showing seven groups" width="720">
...
1. Add a `PyatsCredential` at **PyATS/Genie → Credentials → Add Credential**. Pick a device, enter username + password (+ optional enable secret). The secrets are encrypted with Fernet before they hit the database.
...
4. When the worker finishes, the snapshot appears in the tab's recent-snapshots list and under **PyATS/Genie → Snapshots**.
