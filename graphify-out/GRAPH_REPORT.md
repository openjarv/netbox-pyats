...
Cohesion: 0.08
Nodes (80): What a :class:`PyatsSnapshot` captures from a device.      ``config`` runs parse, Who/what triggered a snapshot capture.      ``user`` captures are initiated from, SnapshotKindChoices, SnapshotTriggerChoices, Meta, PyatsCredential, PyatsGoldenConfig, PyatsParserCatalog (+72 more)

### Community 1 - "refresh_parser_catalog_for_os"
Cohesion: 0.05
Nodes (30): CatalogRefreshResult, Parser-catalog refresh core — the Genie work, isolated from NetBox/RQ.  :func:`r, Return the deduplicated set of Genie-supported pyATS os strings.      Derived fr, Outcome of a single :func:`refresh_parser_catalog_for_os` call.      The :func:`, Return ``(genie_version, pyats_version)`` from the worker environment.      Best, Build a minimal ``pyats.topology.Device`` with only ``.os`` set.      ``genie.li, Discover the parseable command list for one pyATS os.      Worker-only: lazily i, refresh_parser_catalog_for_os() (+22 more)
...
Cohesion: 0.08
Nodes (26): Return ``(platform_supported, os_value)`` for the device-page panel.      Combin, resolve_panel_platform_support(), group_snapshots_by_kind(), Pure-Python helpers for the device-page PyATS tab (ATW-393, ADR-0007).  This mod, Group snapshots by ``kind`` for the diff picker (ATW-241 child 4).      Returns, FakeSnapshot, QA-independent verification for the ATW-252 diff picker kind filter.  Written by, Render the device-tab diff-picker partial and assert the ATW-252     contract: o (+18 more)

### Community 5 - "DiffStatusChoices"
Cohesion: 0.13
Nodes (45): CredentialProtocolChoices, CredentialScopeChoices, DiffStatusChoices, GoldenConfigSourceChoices, PyatsJobStatusChoices, PyatsJobTypeChoices, How a :class:`PyatsGoldenConfig` row was authored (Phase 4, ATW-15).      ``manu, How a credential is assigned.      ``device`` credentials attach to a single Net (+37 more)

### Community 6 - "PyatsSnapshotDiff"
Cohesion: 0.10
Nodes (35): Meta, Meta, PyatsCaptureScheduleType, PyatsCredentialType, PyatsJobType, PyatsParserCatalogRefreshScheduleType, PyatsParserCatalogType, PyatsSnapshotDiffType (+27 more)

### Community 7 - "SnapshotStatusChoices"
Cohesion: 0.07
Nodes (22): Choice sets for the netbox-pyats plugin., Outcome of a snapshot capture attempt.      ``success`` means a JSONB ``data`` p, SnapshotStatusChoices, Migration, Migration, Migration, Migration, Migration (+14 more)

### Community 8 - "capture_snapshot"
Cohesion: 0.13
Nodes (9): capture_snapshot(), Capture a snapshot from a single, already-connected pyATS Device.      This is t, FakePyatsDevice, kind='parse' runs device.parse() per user-supplied command and writes     the sa, Duck-typed pyATS Device for capture tests.      Only the attributes/methods :fun, TestBadKind, TestConfigCapture, TestParseCapture (+1 more)
...
### Community 10 - "PyatsJob"
Cohesion: 0.12
Nodes (27): JobRunner, Recurring dispatcher for capture schedules (ATW-433, ADR-0008).      A registere, Recurring dispatcher for the parser catalog refresh (ATW-581).      A registered, RunCaptureSchedulesJob, RunParserCatalogRefreshSchedulesJob, PyatsJob, PyatsParserCatalogRefreshSchedule, Opt-in intent model for recurring parser-catalog refresh (ATW-581).      A singl (+19 more)
...
Cohesion: 0.11
Nodes (15): Meta, PyatsComplianceRun, One compliance check result: golden config vs. captured snapshot (Phase 4, ATW-1, _AppendOnlyListViewsBase, TestCase, PyatsComplianceRunListViewRenderTest, PyatsJobListViewRenderTest, PyatsSnapshotDiffListViewRenderTest (+7 more)
...
### Community 20 - "test_template_extension.py"
Cohesion: 0.09
...
### Community 27 - "PyatsComplianceRunModelTest"
Cohesion: 0.15
...
Cohesion: 0.10
Nodes (8): TestCase, Tests for the supported-platforms report (Phase 5, ATW-16, Option A).  Two lanes, Report contents: the static map renders with per-slug device counts., The static map the report renders (Phase 5, ATW-16, Option A)., ADR-0001 §6: the data path the report view reads must not import Genie.      The, SupportedPlatformsReportViewTest, TestSupportedPlatformsMap, TestSupportedPlatformsReportWebProcessSafety

### Community 29 - "crypto.py"
Cohesion: 0.12
Nodes (16): decrypt(), _derive_fernet_key_from_secret_key(), encrypt(), _get_config(), get_fernet_key(), is_encrypted_token(), Encryption helpers for the plugin-local PyATS credential store.  Field-level enc, Decrypt a Fernet token produced by :func:`encrypt`.      Empty input round-trips (+8 more)
...
### Community 33 - "ComplianceResultChoices"
Cohesion: 0.17
Nodes (15): ComplianceModeChoices, ComplianceResultChoices, Outcome of a compliance run (Phase 4, ATW-15).      ``compliant`` means the devi, How :func:`netbox_pyats.compliance.run_compliance` compares configs.      ``orde, ComplianceResult, Compliance engine — golden config vs. snapshot raw config diff (Phase 4, ATW-15), Outcome of a single :func:`run_compliance` call.      The RQ job (:func:`netbox_, Length of the JSON-serialized ``diff`` payload, in bytes. (+7 more)
...
### Community 35 - "run_diff_job"
Cohesion: 0.16
Nodes (17): BaseException, enqueue_parse(), _finish_success(), parse_commands_job(), _persist_error_row(), _persist_input_error_row(), Enqueue an on-demand parse job on the dedicated ``pyats`` RQ queue.      This is, RQ worker entry point — run on-demand parses and persist the snapshot.      NetB (+9 more)
...
Cohesion: 0.12
Nodes (11): DuplicateDeviceError, _fake_testbed_factory(), FakeDeviceType, FakeIPAddress, FakeManufacturer, Exception, Tests for :mod:`netbox_pyats.testbed`.  Pure-Python: exercises the NetBox→pyATS, Duck-type stand-in for ``pyats.topology.testbed.DuplicateDeviceError``.      The (+3 more)
...
### Community 39 - "PyatsComplianceRunViewTest"
Cohesion: 0.12
...
### Community 41 - "extract_snapshot_raw_config"
Cohesion: 0.21
...
### Community 42 - "DeviceBulkCaptureView"
Cohesion: 0.16
Nodes (12): DeviceBulkCaptureView, DeviceCaptureView, DeviceDiffView, DeviceParseView, DeviceRefreshCatalogView, Endpoint the device-page PyATS panel POSTs to.      Accepts a ``kind`` (config /, Endpoint the device-page PyATS panel POSTs to.      Accepts ``before_id`` and ``, Bulk "PyATS capture" action on the NetBox device list (Phase 5, ATW-16).      Th (+4 more)

### Community 43 - "ADR-0004: Compliance golden-config comparison shape"
Cohesion: 0.13
...
### Community 44 - "Contributing to netbox-pyats"
Cohesion: 0.13
...
### Community 47 - "PyatsGoldenConfigAPITest"
Cohesion: 0.14
...
### Community 49 - "dev-seed.sh"
Cohesion: 0.27
Nodes (10): cmd_build(), cmd_force_restore(), cmd_info(), cmd_remove(), cmd_restore(), die(), _restore(), dev-seed.sh script (+2 more)

### Community 50 - "ADR-0006: PR-body hygiene — no Paperclip control-plane metadata in public GitHub artifacts"
Cohesion: 0.15
...
### Community 51 - "Remote access to the dev NetBox UI over Tailscale"
Cohesion: 0.15
...
### Community 56 - "ADR-0002: Multi-vendor graceful degradation pattern"
Cohesion: 0.18
...
### Community 57 - "Graphify MCP HTTP server — multi-host / shared-service runbook"
Cohesion: 0.18
...
### Community 58 - "PyatsCredentialModelTest"
Cohesion: 0.18
Nodes (3): TestCase, PyatsCredentialModelTest, Field-level encryption and validation behavior of PyatsCredential.

### Community 59 - "ADR-0003: NetBox 4.6 migration dependencies and worker build toolchain"
Cohesion: 0.20
...
### Community 60 - "ADR-0005: PyatsJob unified job-tracking model + status vocabulary extension"
Cohesion: 0.20
...
### Community 66 - "resolve_state_commands"
Cohesion: 0.33
...
### Community 74 - "_build_device_entry"
Cohesion: 0.25
...
### Community 85 - "ADR-0007: Device-page tab via `register_model_view` + `ObjectView`"
Cohesion: 0.33
...
### Community 88 - "_mark_running"
Cohesion: 0.33
Nodes (6): enqueue_refresh_parser_catalog(), _mark_running(), Enqueue a parser-catalog refresh job on the dedicated ``pyats`` RQ queue.      T, Set a :class:`PyatsJob` to ``running`` with ``started_at=now()``.      Called at, RQ worker entry point — refresh PyatsParserCatalog rows for every os.      NetBo, refresh_parser_catalog_job()

### Community 89 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 90 - "graphify-mcp-key.sh"
Cohesion: 0.53
Nodes (4): ensure_gitignored(), fingerprint_key(), graphify-mcp-key.sh script, usage()

### Community 91 - "netbox-pyats documentation"
Cohesion: 0.40
...
- **Why does `SnapshotKindChoices` connect `PyatsSnapshot` to `resolve_panel_platform_support`, `DiffStatusChoices`, `PyatsSnapshotDiff`, `SnapshotStatusChoices`, `capture_snapshot`, `PyatsJob`, `CaptureResult`, `run_capture_schedules_job`, `PyatsComplianceRun`, `PyatsSnapshotDiffModelTest`, `jobs.py`, `DeviceDiffFormKindFilterTest`, `test_pyatsjob.py`, `PyatsComplianceRunModelTest`, `ComplianceResultChoices`, `PyatsComplianceRunViewTest`, `DeviceBulkCaptureView`, `DeviceDiffForm`, `PyatsGoldenConfigAPITest`, `capture.py`, `test_capture.py`, `resolve_state_commands`, `DiffTableRenderTest`, `TestStateCapture`, `TestPerOsStateCapture`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
- **Why does `PyatsSnapshot` connect `PyatsSnapshot` to `views.py`, `DiffStatusChoices`, `PyatsSnapshotDiff`, `SnapshotStatusChoices`, `PyatsJob`, `CaptureResult`, `PyatsComplianceRun`, `PyatsSnapshotDiffModelTest`, `jobs.py`, `DeviceDiffFormKindFilterTest`, `test_pyatsjob.py`, `PyatsComplianceRunModelTest`, `ComplianceResultChoices`, `run_diff_job`, `PyatsComplianceRunViewTest`, `DeviceBulkCaptureView`, `DeviceDiffForm`, `PyatsGoldenConfigAPITest`, `DiffTableRenderTest`, `.get_status_color`, `.has_warnings`?**
  _High betweenness centrality (0.046) - this node is a cross-community bridge._
- **Why does `PyatsJob` connect `PyatsJob` to `PyatsSnapshot`, `ComplianceResultChoices`, `views.py`, `refresh_parser_catalog_for_os`, `DiffStatusChoices`, `PyatsSnapshotDiff`, `SnapshotStatusChoices`, `DeviceBulkCaptureView`, `DeviceDiffForm`, `CaptureResult`, `PyatsComplianceRun`, `jobs.py`, `test_pyatsjob.py`, `.get_status_color`, `.related_result`?**
