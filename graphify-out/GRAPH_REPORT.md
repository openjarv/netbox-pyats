...
## Summary
- 1951 nodes · 5217 edges · 147 communities (101 shown, 46 thin omitted)
- Extraction: 62% EXTRACTED · 38% INFERRED · 0% AMBIGUOUS · INFERRED: 1986 edges (avg confidence: 0.5)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `a6c747b7`
...
1. `PyatsSnapshot` - 205 edges
2. `PyatsJob` - 189 edges
3. `PyatsSnapshotDiff` - 182 edges
4. `PyatsComplianceRun` - 165 edges
5. `PyatsCredential` - 161 edges
6. `PyatsGoldenConfig` - 159 edges
7. `PyatsCaptureSchedule` - 157 edges
8. `SnapshotKindChoices` - 150 edges
9. `PyatsParserCatalogRefreshSchedule` - 145 edges
10. `PyatsParserCatalog` - 124 edges
...
Cohesion: 0.07
Nodes (97): What a :class:`PyatsSnapshot` captures from a device.      ``config`` runs parse, Who/what triggered a snapshot capture.      ``user`` captures are initiated from, SnapshotKindChoices, SnapshotTriggerChoices, PyatsCredential, PyatsGoldenConfig, PyatsParserCatalog, PyatsParserCatalogRefreshSchedule (+89 more)

### Community 1 - "PyatsComplianceRun"
Cohesion: 0.11
Nodes (62): ComplianceModeChoices, ComplianceResultChoices, CredentialProtocolChoices, CredentialScopeChoices, DiffStatusChoices, GoldenConfigSourceChoices, PyatsJobStatusChoices, PyatsJobTypeChoices (+54 more)

### Community 2 - "PyatsSnapshot"
Cohesion: 0.08
...
### Community 7 - "CaptureResult"
Cohesion: 0.08
Nodes (16): CaptureResult, Outcome of a single :func:`capture_snapshot` call.      The :class:`~netbox_pyat, Length of the JSON-serialized ``data`` payload, in bytes., TestCaptureResultSizeBytes, BatchCaptureJobTest, CaptureJobPyatsJobPlumbingTest, DeviceBulkCaptureViewTest, DiffJobPyatsJobPlumbingTest (+8 more)
...
### Community 13 - "group_snapshots_by_kind"
Cohesion: 0.10
...
### Community 17 - "PyatsSnapshotDiff"
Cohesion: 0.11
Nodes (13): Meta, PyatsSnapshotDiff, One structured diff between two :class:`PyatsSnapshot` rows of a device.      Po, _AppendOnlyListViewsBase, PyatsComplianceRunListViewRenderTest, PyatsJobListViewRenderTest, PyatsSnapshotDiffListViewRenderTest, PyatsSnapshotListViewRenderTest (+5 more)
...
Cohesion: 0.12
Nodes (23): batch_capture_job(), _create_pyats_job(), enqueue_batch_capture(), enqueue_capture(), enqueue_compliance(), enqueue_diff(), enqueue_learn(), enqueue_parse() (+15 more)
...
### Community 23 - "SnapshotStatusChoices"
Cohesion: 0.09
Nodes (12): Outcome of a snapshot capture attempt.      ``success`` means a JSONB ``data`` p, SnapshotStatusChoices, _FakeModuleInfo, FakeOpsClassModule, FakeOpsFeatureModule, Duck-typed ``pkgutil.ModuleInfo`` for ``pkgutil.iter_modules``., Duck-typed 2nd-level ``lookup.ops.<feature>.<feature>`` module.      Exposes the, Duck-typed 1st-level ``lookup.ops.<feature>`` AbstractedModule.      Exposes the (+4 more)

### Community 24 - "DeviceParseViewTest"
Cohesion: 0.09
Nodes (7): DeviceParseFormTest, DeviceParseViewTest, DeviceRefreshCatalogViewTest, Tests for the device-page Parse sub-tab (ATW-241 child 2, ATW-250).  Requires a, View tests for :class:`views.DeviceRefreshCatalogView` (ATW-250)., Pure-form validation for :class:`forms.DeviceParseForm`., View tests for :class:`views.DeviceParseView` (ATW-250).

### Community 25 - ".get"
Cohesion: 0.11
Nodes (9): _diff_list_url(), _genie_diff_post_url(), Return the most recent snapshots for a device (or empty list)., Return the POST URL for the Genie Diff page diff form., Return the full diff history list URL (pyatssnapshotdiff_list)., Resolve the pyATS os + catalog row + command choices for a device.      Web-proc, Return the POST URL for the device-page "Refresh parser list" button., _refresh_parser_catalog_url_for_device() (+1 more)
...
Cohesion: 0.12
Nodes (11): Exception, DuplicateDeviceError, _fake_testbed_factory(), FakeDeviceType, FakeIPAddress, FakeManufacturer, Tests for :mod:`netbox_pyats.testbed`.  Pure-Python: exercises the NetBox→pyATS, Duck-type stand-in for ``pyats.topology.testbed.DuplicateDeviceError``.      The (+3 more)
...
### Community 45 - "platform_to_pyats_os"
Cohesion: 0.31
...
### Community 46 - "GenieDiffViewTest"
Cohesion: 0.13
...
### Community 47 - "GenieParseViewTest"
Cohesion: 0.13
...
### Community 52 - "CatalogRefreshResult"
Cohesion: 0.32
...
### Community 53 - "PyatsSnapshotDiffModelTest"
Cohesion: 0.22
...
### Community 56 - "GenieLearnViewTest"
Cohesion: 0.17
...
### Community 57 - "_extract_snapshot_raw"
Cohesion: 0.27
...
### Community 59 - "PyatsSnapshotModelTest"
Cohesion: 0.17
Nodes (3): PyatsSnapshotModelTest, Tests for :class:`netbox_pyats.models.PyatsSnapshot`.  Requires a running NetBox, Persistence and helper behavior of PyatsSnapshot.
...
### Community 62 - "ADR-0002: Multi-vendor graceful degradation pattern"
Cohesion: 0.18
...
### Community 63 - "Graphify MCP HTTP server — multi-host / shared-service runbook"
Cohesion: 0.18
...
### Community 65 - "TestbedBuildReport"
Cohesion: 0.20
...
### Community 66 - "ADR-0003: NetBox 4.6 migration dependencies and worker build toolchain"
Cohesion: 0.20
...
### Community 67 - "ADR-0005: PyatsJob unified job-tracking model + status vocabulary extension"
Cohesion: 0.20
...
### Community 73 - "DiffTableRenderTest"
Cohesion: 0.31
...
### Community 90 - "supported_os_values"
Cohesion: 0.38
...
### Community 92 - "ADR-0007: Device-page tab via `register_model_view` + `ObjectView`"
Cohesion: 0.33
...
### Community 98 - "RunCaptureSchedulesJob"
Cohesion: 0.40
...
### Community 102 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 103 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
...
- **Why does `SnapshotKindChoices` connect `PyatsCredential` to `PyatsComplianceRun`, `PyatsSnapshot`, `run_capture_schedules_job`, `capture_snapshot`, `CaptureResult`, `DeviceDiffForm`, `group_snapshots_by_kind`, `capture.py`, `test_capture_learn.py`, `PyatsSnapshotDiff`, `jobs.py`, `SnapshotStatusChoices`, `PyatsGoldenConfigAPITest`, `choices.py`, `PyatsSnapshotDiffModelTest`, `PyatsComplianceRunModelTest`, `PyatsSnapshotModelTest`, `FakeOpsInstance`, `PyatsJobModelTest`, `DiffTableRenderTest`, `PyatsGoldenConfigModelTest`, `PyatsComplianceRunViewTest`, `RunCaptureSchedulesJob`, `RunDiffJobDoesNotExistTest`?**
  _High betweenness centrality (0.058) - this node is a cross-community bridge._
- **Why does `PyatsSnapshot` connect `PyatsSnapshot` to `PyatsCredential`, `PyatsComplianceRun`, `views.py`, `.get_status_color`, `.has_warnings`, `CaptureResult`, `DeviceDiffForm`, `PyatsSnapshotDiff`, `jobs.py`, `run_diff_job`, `SnapshotStatusChoices`, `PyatsGoldenConfigAPITest`, `GenieDiffViewTest`, `GenieParseViewTest`, `PyatsSnapshotDiffModelTest`, `PyatsComplianceRunModelTest`, `GenieLearnViewTest`, `PyatsSnapshotModelTest`, `PyatsJobModelTest`, `DiffTableRenderTest`, `PyatsGoldenConfigModelTest`, `PyatsComplianceRunViewTest`, `RunCaptureSchedulesJob`, `RunDiffJobDoesNotExistTest`?**
  _High betweenness centrality (0.054) - this node is a cross-community bridge._
- **Why does `PyatsJob` connect `PyatsSnapshot` to `PyatsCredential`, `PyatsComplianceRun`, `RunCaptureSchedulesJob`, `views.py`, `.get_status_color`, `.related_result`, `PyatsJobModelTest`, `CaptureResult`, `DeviceDiffForm`, `PyatsSnapshotDiff`, `jobs.py`, `CatalogRefreshResult`, `SnapshotStatusChoices`, `PyatsParserCatalogModelTest`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Are the 159 inferred relationships involving `PyatsSnapshot` (e.g. with `Meta` and `PyatsCaptureScheduleSerializer`) actually correct?**
  _`PyatsSnapshot` has 159 INFERRED edges - model-reasoned connections that need verification._
- **Are the 146 inferred relationships involving `PyatsJob` (e.g. with `Meta` and `PyatsCaptureScheduleSerializer`) actually correct?**
  _`PyatsJob` has 146 INFERRED edges - model-reasoned connections that need verification._
- **Are the 149 inferred relationships involving `PyatsSnapshotDiff` (e.g. with `Meta` and `PyatsCaptureScheduleSerializer`) actually correct?**
  _`PyatsSnapshotDiff` has 149 INFERRED edges - model-reasoned connections that need verification._
- **Are the 136 inferred relationships involving `PyatsComplianceRun` (e.g. with `Meta` and `PyatsCaptureScheduleSerializer`) actually correct?**
  _`PyatsComplianceRun` has 136 INFERRED edges - model-reasoned connections that need verification._