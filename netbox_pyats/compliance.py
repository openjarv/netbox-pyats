...
:func:`netbox_pyats.diff.diff_snapshots`. It takes a golden-config text and a
captured snapshot's ``data`` payload, derives the comparable "golden config
dict" from the golden text, diffs it against the snapshot's parsed config
payload using the Phase 3 :func:`netbox_pyats.diff.diff_snapshots` engine, and
returns a :class:`ComplianceResult` classified as ``compliant`` / ``drift`` /
``error``.
...
``show running-config`` via ``device.parse(...)``). To compare the golden text
against it in the same shape, we parse the golden text into a comparable dict
on the worker (when a connected pyATS Device is available to drive the Genie
parser), or — when no device is available — fall back to a line-oriented
golden-config dict that the caller supplies. v1 keeps this module pure-Python
...
def run_compliance(
    ...
    *,
    ...
    a dict on the worker (when a device is available to drive the Genie parser),
    loading the :class:`PyatsSnapshot` row, extracting its
    ``data["config"]`` payload, and persisting the returned
    :class:`ComplianceResult`. This function only runs the diff + classification.
