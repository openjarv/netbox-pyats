...
> Replace the `<...>` placeholders below with your own dev host's Tailscale
> values (`tailscale ip -4`, `tailscale status`). This runbook never hardcodes
> any host-specific facts — every example uses placeholders you must fill in.
